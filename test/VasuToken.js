  ...
  it("transfers token ownership", function() {
    return VasuToken.deployed()
      ...
      .catch(function(error) {
        assert(error, "Expected an error but did not get one"); // Fix here
      ...
      .then(function(receipt) {
        ...
        assert.equal(receipt.logs[0].args._to, accounts[1], "logs receiver");
        assert.equal(receipt.logs[0].args._value, 250000, "logs amount");
        return tokenInstance.balanceOf(accounts[1]);
      })
      .then(function(balance) {
        assert.equal(balance.toNumber(), 250000, "adds amount to receiver");
        return tokenInstance.balanceOf(accounts[0]);
      })
      .then(function(balance) {
